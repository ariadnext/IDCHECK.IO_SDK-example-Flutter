...
class IDCheckio {
  ...
  static Future<void> activate({@required String licenceFilename, Environment environment, bool disableAudioForLiveness, bool disableImeiForActivation, bool extractData}) async {
    try {
  ...
  static Future<IDCheckioResult> startOnline(IDCheckioParams params, CISContext cisContext) async {
    try {
      String json = await _channel.invokeMethod('startOnline', <String, dynamic>{
        'params': params.toJson(),
        if (cisContext != null) 'cisContext': cisContext.toJson() else 'cisContext': null,
      });
      IDCheckioResult result = IDCheckioResult.fromJson(jsonDecode(json));
      return result;
    } on PlatformException catch (e) {
  ...
  static Future<IDCheckioResult> analyze({@required IDCheckioParams params, @required String side1Uri, String side2uri, bool isOnline, CISContext cisContext}) async {
    try {
...
class IDCheckioResult {
  ...
  IDCheckioResult.fromJson(Map<String, dynamic> json) {
    ...
    if (error != null) {
      sessionError = ErrorMsg.fromJson(error);
    }
  }
  ...
  String toJson() {
    ...
    String folder = (folderUid != null) ? ",\"folderUid\":\"${this.folderUid}\"" : "";

    String task = (taskUid != null) ? ",\"taskUid\":\"${this.taskUid}\"" : "";

    String resultDocumentUid = (documentUid != null) ? ",\"documentUid\":\"${this.documentUid}\"" : "";

    String taskUidsList = "";
    if (attachmentTaskUids != null) {
      for (String uid in attachmentTaskUids) {
        taskUidsList += "\"$uid\",";
      }
    }
    if (taskUidsList != null && taskUidsList.length > 0) {
      taskUidsList = taskUidsList.substring(0, taskUidsList.length - 1);
    }

    if (taskUidsList.isNotEmpty) {
      taskUidsList = ",\"attachmentTaskUids\":[$taskUidsList]";
    }

    String documentUidsList = "";
    if (attachmentDocumentUids != null) {
      for (String uid in attachmentTaskUids) {
        documentUidsList += "\"$uid\",";
      }
    }
    if (documentUidsList != null && documentUidsList.length > 0) {
      documentUidsList = documentUidsList.substring(0, documentUidsList.length - 1);
    }

    if (documentUidsList.isNotEmpty) {
      documentUidsList = ",\"attachmentDocumentUids\":[$documentUidsList]";
    }

    return "{\"images\":[$resultImages]," + "\"document\":{${document?.toJson() ?? ""}}" + folder + task + resultDocumentUid + taskUidsList + documentUidsList + "}";
  }
}
...
class ImageResult {
  ...
  String toJson() => "{\"source\":\"$source\",\"cropped\":\"$cropped\",\"face\":\"$face\",\"imageStatus\":\"$imageStatus\"}";
}
...
class IdentityDocument extends Document {
  ...
  String toJson() {
    ...
    fields.forEach((k, v) => resultFields += "\"${k.toString().trim()}\":${v.toJson()},");

    if (resultFields != null && resultFields.length > 0) {
      resultFields = resultFields.substring(0, resultFields.length - 1);
    }

    return "\"type\":\"IdentityDocument\"," + "\"status\":\"${status.toString().toLowerCase()}\"," + "\"fields\":{$resultFields}";
  }
}
...
class VehicleRegistrationDocument extends Document {
  ...
  String toJson() {
    ...
    return "\"type\":\"VehicleRegistrationDocument\"," + "\"status\":\"${status.toString().toLowerCase()}\"," + "\"fields\":{$resultFields}";
  }
}
...
class IDCheckioParams {
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (readEmrtd != null) 'ReadEmrtd': readEmrtd else 'ReadEmrtd': false,
...
class CISContext {
  String folderUid = null;
  String referenceTaskUid = null;
  String referenceDocUid = null;
  CISType cisType = null;
  bool biometricConsent = null;

  Map<String, dynamic> toJson() {
    Map<String, dynamic> result = Map<String, dynamic>();
    if (folderUid != null) result.putIfAbsent('folderUid', () => folderUid);
    if (referenceTaskUid != null) result.putIfAbsent('referenceTaskUid', () => referenceTaskUid);
    if (referenceDocUid != null) result.putIfAbsent('referenceDocUid', () => referenceDocUid);
